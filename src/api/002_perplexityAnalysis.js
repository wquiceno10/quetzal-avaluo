...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return Response.json(
      { error: 'JSON inválido', details: e.message },
      { status: 400 },
    );
  }
...
${formData.informacion_complementaria ? `- Información adicional: ${formData.informacion_complementaria}` : ''}
  ...
  // -----------------------------
  // 3. PROMPT NUEVO PARA PERPLEXITY - CON INSTRUCCIONES DE FILTRADO
  // -----------------------------

  const areaConstruida = formData.area_construida ? parseInt(formData.area_construida) : null;
  const margenArea = 0.30; // ±30%
...
`.trim();
  ...
  try {
    ...
    return Response.json(
      { error: 'Error al llamar a Perplexity', details: err.message },
      { status: 500 },
    );
  }
  ...
  if (!perplexityContent) {
    return Response.json(
      { error: 'Perplexity devolvió una respuesta vacía' },
      { status: 500 },
    );
  }

  // -----------------------------
  // 5. Esquema JSON para InvokeLLM
  // -----------------------------
  ...
  // -----------------------------
  // 6. Prompt de extracción para InvokeLLM - SINCRONIZADO CON PERPLEXITY
  // -----------------------------
...
// -----------------------------
// 7. Llamar a InvokeLLM y procesar SIN FILTROS
// -----------------------------
try {
  ...
  console.log(
    'InvokeLLM Extracted Data (first part):',
...
const crearComparable = (c, yieldMercado) => {
  ...
  // 2. Crear TODOS los comparables que InvokeLLM extrajo
