...
Devuelve SOLO JSON v\xE1lido.
    ...
    if (!esLote || area <= 1e3) {
      ...
      console.log(`Comparables despu\xE9s de filtro de \xE1rea (50-150%): ${comparablesFiltradosPorArea.length}`);
    }
    ...
    if (comparablesParaTabla.length >= 5) {
      ...
      if (preciosM2.length >= 4) {
        ...
        if (filtradosIQR.length >= 5) {
          console.log(`Filtro IQR: ${comparablesParaTabla.length} -> ${filtradosIQR.length} (rango: ${minThreshold.toLocaleString()} - ${maxThreshold.toLocaleString()}/m\xB2)`);
          comparablesParaTabla = filtradosIQR;
        } else {
          console.log(`Filtro IQR no aplicado (quedar\xEDan ${filtradosIQR.length} comparables)`);
        }
      }
    }
    const resultado = {
