...
Para que el sistema procese la informaci\xF3n correctamente, debes presentar el listado de comparables en el siguiente formato de lista (NO tabla):

**T\xEDtulo del inmueble**
Tipo (Venta/Arriendo) | Precio
\xC1rea (m2) | Habitaciones | Ubicaci\xF3n
**Fuente** (solo nombre, sin extensi\xF3n, ej: Fincaraiz)

Ejemplo:
**Apartamento en Condina, Pereira**
Venta | 245.000.000
68 m\xB2 | 3 hab | Condina, Pereira
**Fincaraiz**

*Nota: T\xEDtulo y Precio deben ser lo m\xE1s fieles posible al anuncio original. Separa cada comparable con una l\xEDnea en blanco.*
...
## 4. RESUMEN EJECUTIVO
Cierra con 1-4 p\xE1rrafos claros con el valor recomendado y estrategia de venta.
...
1. "comparables": Extrae CADA INMUEBLE del listado (formato multi-l\xEDnea, NO tabla).
   ...
   **T\xEDtulo**
   Tipo | Precio
   \xC1rea | Habitaciones | Ubicaci\xF3n
   Fuente
   
   Extrae:
   - "titulo": El texto que aparece en negrita (entre **...**)
   - "precio_lista": El n\xFAmero EXACTO del precio (el que aparece despu\xE9s del | en la segunda l\xEDnea)
   - "tipo_operacion": "venta" o "arriendo" (el que aparece ANTES del | en la segunda l\xEDnea)
     * IMPORTANTE: Si dice "Venta", es "venta". Si dice "Arriendo", es "arriendo". NO asumas nada por el precio.
   - "area": \xC1rea en m\xB2 (el n\xFAmero que aparece antes de "m\xB2" en la tercera l\xEDnea)
   - "habitaciones": N\xFAmero de habitaciones (el n\xFAmero que aparece antes de "hab" en la tercera l\xEDnea)
   - "ubicacion": La ubicaci\xF3n completa (el texto despu\xE9s del segundo | en la tercera l\xEDnea)
   - "fuente": El nombre de la fuente (cuarta l\xEDnea)
...
3. "yield_zona": Busca la frase exacta "Yield promedio mercado: X.XX%" en el texto.
   Extrae SOLO el n\xFAmero como decimal (ej: si dice "0.45%", devuelve 0.0045).
...
Devuelve SOLO JSON v\xE1lido.
    ...
    const valorPonderado = valorVentaDirecta && valorRentabilidad && compsArriendo.length > 0 ? Math.round(valorVentaDirecta * 0.6 + valorRentabilidad * 0.4) : null;
