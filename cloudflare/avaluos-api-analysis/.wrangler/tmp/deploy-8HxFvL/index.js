...
// src/index.js
console.log("Deploy V6 (Quality Restoration) - " + (/* @__PURE__ */ new Date()).toISOString());
var index_default = {
...
Tu tarea es realizar un estudio de mercado t\xE9cnico y detallado para un inmueble en: ${ubicacion} (${tipoInmueble}).
Datos Objetivo: \xC1rea ${area}m\xB2, ${formData.habitaciones || "?"} habitaciones.

## 1. B\xDASQUEDA DE COMPARABLES
Busca en portales inmobiliarios reales (Fincaraiz, Metrocuadrado, Ciencuadras, etc.).
- Filtro de \xC1rea: ${minArea}m\xB2 a ${maxArea}m\xB2.
- Cantidad: M\xEDnimo 15 comparables.
- Mix: Intenta que el 20-30% sean ARRIENDOS para calcular rentabilidad.
- Si no hay exactos en el barrio, busca en zonas aleda\xF1as del mismo estrato.

## 2. LISTADO DETALLADO
Para cada inmueble encontrado, genera una l\xEDnea en una TABLA MARKDOWN con:
- T\xEDtulo del Anuncio (Extracto breve)
- Ubicaci\xF3n (Barrio)
...
## 3. AN\xC1LISIS DE MERCADO
Calcula y explica:
- Precio promedio por m\xB2 en VENTA en la zona.
- Canon de arrendamiento promedio para esta tipolog\xEDa.
- Yield (Rentabilidad) mensual estimada del sector (ej: 0.5%).

## 4. RESUMEN EJECUTIVO
Escribe 2 p\xE1rrafos densos y profesionales dirigidos al propietario con:
- Valoraci\xF3n del mercado actual (oferta/demanda).
- Rango de precios sugerido para venta y arriendo.
- Factores clave que valorizan o desvalorizan en esta zona espec\xEDfica.

IMPORTANTE:
- Usa fuentes reales.
- S\xE9 preciso con los n\xFAmeros.
- NO inventes datos.
        `.trim();
    ...
    try {
      const response = await fetch("https://api.perplexity.ai/chat/completions", {
        ...
        body: JSON.stringify({
          model: "sonar",
          // Mantenemos sonar por velocidad, pero con prompt detallado
          messages: [
...
2. "resumen_mercado": Extrae EL TEXTO COMPLETO de la secci\xF3n "RESUMEN EJECUTIVO". Queremos el an\xE1lisis rico y detallado.
...
Devuelve SOLO JSON v\xE1lido.
    ...
    const comparables = (extractedData.comparables || []).map((c) => {
      ...
      if (precioLista && precioLista < 2e7) {
        esArriendo = true;
      }
